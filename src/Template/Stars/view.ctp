<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Star $star
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Star'), ['action' => 'edit', $star->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Star'), ['action' => 'delete', $star->id], ['confirm' => __('Are you sure you want to delete # {0}?', $star->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Stars'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Star'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Users'), ['controller' => 'Users', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New User'), ['controller' => 'Users', 'action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Posts'), ['controller' => 'Posts', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Post'), ['controller' => 'Posts', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="stars view large-9 medium-8 columns content">
    <h3><?= h($star->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('User') ?></th>
            <td><?= $star->has('user') ? $this->Html->link($star->user->id, ['controller' => 'Users', 'action' => 'view', $star->user->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Post') ?></th>
            <td><?= $star->has('post') ? $this->Html->link($star->post->id, ['controller' => 'Posts', 'action' => 'view', $star->post->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($star->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Star Score') ?></th>
            <td><?= $this->Number->format($star->star_score) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($star->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($star->modified) ?></td>
        </tr>
    </table>
</div>
